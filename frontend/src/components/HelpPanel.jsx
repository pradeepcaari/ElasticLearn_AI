...
// ─── Preprocess multi-line $$ blocks into single lines ────────────────────────
...
// ─── Simple markdown renderer (bold, code, blank lines, math) ─────────────────
function SimpleMarkdown({ text }) {
  const lines = preprocessMathBlocks(text);
  return (
    <div style={{ fontSize:"0.9rem", lineHeight:1.8, color:"var(--text)" }}>
      {lines.map((line, i) => {
        const trimmed = line.trim();
        if (line.startsWith("```")) return <div key={i} style={{ height:4 }} />;
        // Block math: standalone $$...$$ on one line
        if (trimmed.startsWith("$$") && trimmed.endsWith("$$") && trimmed.length > 4) {
          return <BlockMath key={i} latex={trimmed.slice(2, -2).trim()} />;
        }
        ...
        return (
          <div key={i} style={{ marginBottom: line === "" ? 10 : 2 }}>
            {parts.map((p, j) => {
              ...
              if (hasMath) return <MathText key={j}>{p}</MathText>;
...
// ─── Help Panel ───────────────────────────────────────────────────────────────
export default function HelpPanel({ topic, mode, sessionId, sessionTitle, onClose }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // ── Real topic explanation from backend ──
        const res  = await fetch("/api/learning/help", {
          method:  "POST",
          headers: {
        ...
        if (!cancelled) setContent(data.response);
      } catch {
