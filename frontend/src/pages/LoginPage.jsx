...
const PAGE_CSS = `
  ...
  .login-input:focus {
    ...
    box-shadow: 0 0 0 3px var(--gold-dim), inset 0 1px 0 rgba(201,168,76,0.03);
  }
...
function LeftPanel() {
  ...
  return (
    <div className="login-left" style={{
      ...
      position: "relative",
      overflow: "hidden",
      background: `radial-gradient(ellipse 80% 60% at 30% 40%, var(--login-bg-glow) 0%, transparent 65%), var(--navy)`,
...
export default function LoginPage({ onLogin }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
       ...
       * Note: backend does NOT return a token on register — we call login next.
       *
       * ── LOGIN ─────────────────────────────────────────────────────────────
       * API: POST /api/auth/login
       * Body: { email: string, password: string }
       * Response (200): { access_token: string, username: string, email: string }
       */

      // ── Register then Login (real backend) ──
      if (!isLogin) {
      ...
      if (!loginRes.ok) throw new Error(data.msg || "Authentication failed");
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        // Store Google token for reference
        ...
        if (!backendRes.ok) {
          throw new Error(backendData.msg || "Google authentication failed with backend");
        }
        
        // Store backend JWT token
        localStorage.setItem("el_token", backendData.access_token);
